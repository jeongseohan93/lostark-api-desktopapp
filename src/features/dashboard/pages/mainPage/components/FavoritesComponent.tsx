    ...
    if (favorites.length === 0) {
        return (
            <div className={style.favoritesBox}>
                ...
                <span className={style.hint}>거래소에서 ★ 버튼으로 아이템을 추가하세요.</span>
            </div>
        );
    }
            ...
            <div className={style.titleRow}>
                ...
                {lastPolled && (
                    <span className={style.badge}>
                        {lastPolled.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })} 수집
                    </span>
                )}
                    ...
                    return (
                        <div key={fav.id} className={style.itemChip}>
                            <span className={style.chipName} style={{ color: GRADE_COLOR[fav.grade] ?? '#888' }}>
                                {fav.name.length > 8 ? fav.name.slice(0, 8) + '…' : fav.name}
                            </span>
